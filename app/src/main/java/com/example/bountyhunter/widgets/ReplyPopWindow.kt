package com.example.bountyhunter.widgets
...
object ReplyPopWindow {
    ...
    fun with(context: Context) {
        ...
        ReplyPopWindow.context = WeakReference(context)
    ...
    fun setReplyName(name: String, content: String) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.N) {
            ...
            val s =
                "回复 <font color=\"#0000FF\">@${name}</font> : ${content.take(6) + if (content.length > 6) "..." else ""}"
    ...
    fun show(view: View, alignMode: AlignMode, offsetYReverse: Int) {
        ...
        replyPopWindow?.get()?.showAsDropDown(
            view, offsetX, -(offsetYReverse + View.MeasureSpec.getSize(
                mainView?.get()!!.measuredHeight
            ) + view.height), Gravity.START
        )
    }
